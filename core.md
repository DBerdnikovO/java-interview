...
+ В сгенерированном файле .class для классов, полей и методов ставится флаг ACC_FINAL; 
+ Для статических констант (static final), если они являются константами во время компиляции (примитивы и строки), в описание поля добавляется атрибут ConstantValue, и значение подтягивается из константного пула при загрузке класса.

+ Для final полей определен особый механизм "безопасной публикации", все записи в такие поля замораживаются при выходе из конструктора и становятся видны другим потокам без дополнительной синхронизации.
