...
Лямбда-выражение, начиная с java 8, не генерируют анонимный класс, а используется invokedynamic + LambdaMetafactory.
...
## Что такое `Stream`?
Интерфейс `java.util.Stream` представляет собой последовательность элементов, над которой можно производить различные операции. Stream не является структурой данных, которая хранит элементы. Он лишь передает элементы из источника через конвейер вычслительных операций.

Операции над стримами бывают или _промежуточными (intermediate)_ или _конечными (terminal)_. Конечные операции возвращают результат определенного типа, а промежуточные операции возвращают новый поток, при этом он связан со старым. Таким образом вы можете строить цепочки из несколько операций над одним и тем же стримом. 
Конвейер потока всегда состоит из источника (коллекции, массива, функции-генератора, канала ввода-вывода), нуля или более промежуточных операций и одной терминальной операции. 
...
Ленивое выполнение позволяет значительно повысить эффективность, например, в конвейере filter -> map -> sum все три операции могут быть объединены в один проход по данным с минимальным количеством структур. 
Ленивость также позволяет обрабатывать данные не полностью, например при операции "найти первую строку длиннее 1000 символов", будет достаточно провреить лишь несколько строк, а не все.
Это особенно важно для бесконечных потоков, где полный обход не возможен.
...
Возможна бесконечность, коллекции всегда имеют конечный размер, а потоки - не обязательно. Коротко-замыкающие операции (например, limit(), findFirs()) позволяют завершить вычисления даже на бесконечных потоках за конечное время.

Элементы потока можно пройти только один раз за время его жизни. Чтобы снова обработаеть те же элементы, придется создавать новый поток.

[к оглавлению](#java-8)

## AutoCloseable класс, сравнение с Closeable. 
__AutoCloseable__ - это интерфейс для объектов, в том числе используемый в Stream, которые используют внешние ресурсы (файл, сетевое подключение, поток данных) и должны освободить их, когда они больше не нужны.
Чтобы не делать этого вручную, Java позволяет использовать конструкцию try-with-resources. Если объект реализует AutoCloasable, то метод close() будет вызван автоматически в конце такого блока, и ресурс гарантировано закроется. Не являетя идемпотентным, в отличие от Closeable (Идемпотентный метод - это метод, который при повторном вызове дает тот же результат и не вызывает дополнительных эффектов), то есть при повторном вызове, мы можем вызвать исключение или выполнить какие-либо дополнительные действия.

[к оглавлению](#java-8)

## Stateful и stateless операции.
Stateless - обрабатывают каждый элемент независимо от других, не сохраняя контекст.
Stateful - учитывают уже обработанные элементы. Например, чтобы отсортировать поток, нужно сначала увидеть все его элементы. В параллельных вычислениях это может потребовать доплнительных проходов или буферизации большого количества данных.

[к оглавлению](#java-8)

## Какие существуют способы создания стрима?
1. Из коллекции через методы stream() и parallelStream():
```java 
Stream<String> fromCollection = Arrays.asList("x", "y", "z").stream();
```
2. Из статических фабричных методов классов stream, набора значений:
```java 
...
```
9. С помощью `BufferedReader.lines()`:
```java 
